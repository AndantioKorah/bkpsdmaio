...
<head>
	...
	<title>Verifikasi Dokumen - Siladen</title>
	<!-- Favicon -->
...
<body>
	...
	<div class="main-wrapper">
			...
			<div class="content container-fluid">
				...
				<div class="page-header">
					<div class="row align-items-center">
						<div class="col">
							...
							<ul class="breadcrumb">
								<li class="breadcrumb-item active">BKD</li>
								<li class="breadcrumb-item active">Verifikasi Dokumen Kepegawaian BKD</li>
							</ul>
						</div>
					</div>
				</div>
				...
				<div class="card">
					<div class="card-header">
						<h4 class="card-title mb-0">Verifikasi Dokumen Kepegawaian Berdasarkan NIP dan Status</h4>
					</div>
					<div class="col-lg-12">
						<form name="frmDaftar" action="#" class="">
							<div class="card-body">
								...
								<div class="form-group row">
									...
									<div class="col-lg-10">
										<select id="unor" name="unor" class="form-control floating unor">
											<option value="">--</option>
										</select>
									</div>
								</div>
								...
								<div class="form-group row">
									...
									<div class="col-md-4">
										<select class="form-control" name="status" id="status">
											<option value="">--</option>
											<?php foreach ($data['status']->result() as $value) : ?>
												<option value="<?php echo $value->id_status ?>" <?php echo ($value->id_status == 3 ? 'selected' : '') ?>><?php echo $value->nama_status ?></option>
											<?php endforeach; ?>
										</select>
									</div>
									<div class="col-md-6"><button type="button" class="btn btn-primary" id="btnTampil">Tampil</button></div>
								</div>
							</div>
		...
		<div class="modal fade" id="vskModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title" id="myModalLabel"><span id="msg"></span></h4>
					...
					<div class="modal-body">
						<form id="nfrmvsk">
							<input type="hidden" name="<?php echo $this->security->get_csrf_token_name() ?>" value="<?php echo $this->security->get_csrf_hash() ?>" style="display: none">
							<p>Anda Yakin akan menyetujui dokumen ini ?</p>
							<input type="hidden" name="id" />

						</form>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-success" id="nBtnVerifikasi">OK Setuju !</button>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$(".unor").select2({
				...
				ajax: {
					url: "<?php echo site_url() ?>/arsip/getUnitOrganisasi",
					dataType: 'json',
					type: 'GET',
					cache: "true",
					delay: 250,
			...
			var table = $('#table').DataTable({
				...
				"deferLoading": 0,
				"ajax": {
					"url": '<?php echo site_url() ?>/arsip/getVerifikasiBKD/',
					"data": function(d) {
						d.cariBy = $('#cariBy').val();
					...
					},
					{
						...
						"render": function(data, type, full, meta) {
							...
							return '<div class="dropdown dropdown-action"><a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>' +
								...
								'<?php if (hasPermission("arsip", "hapus")) : ?><a class="dropdown-item" href="#" data-nip="' + buttonID + '" data-file="' + file + '" data-path="' + path + '" data-toggle="modal" data-target="#dskModal"><i class="fa fa-remove m-r-5"></i> Hapus</a><?php endif ?>' +
								'<?php if (hasPermission("arsip", "verifikasiBKD")) : ?><a class="dropdown-item" href="#" data-id="' + full.raw_name + '"  data-toggle="modal" data-target="#vskModal"><i class="fa fa-check m-r-5"></i> Approve</a><?php endif ?>' +
								'</div></div>';
						}
					}
				]
			...
			$('#vskModal').on('show.bs.modal', function(e) {
				$('#vskModal #msg').text('Konfirmasi Persetujuan Dokumen')
					.removeClass("text-success")
					.removeClass("text-danger")
					.removeClass("text-info");

				var id = $(e.relatedTarget).attr('data-id');
				$('#vskModal input[name=id]').val(id);

				var data = $('#nfrmvsk').serialize();

				$('#vskModal #msg').text('Updating Please Wait.....')
					.removeClass("text-success")
					.addClass("text-info");

				$.ajax({
					type: "POST",
					url: "<?php echo site_url() ?>/arsip/validasiBKD",
					data: data,
					success: function(r) {
						$('#vskModal #msg').text(r.pesan)
							.removeClass("text-info")
							.addClass("text-success");
					...
					error: function(r) {
						$('#vskModal #msg').text(r.responseJSON.pesan)
							.removeClass("text-info")
							.removeClass("text-success")
							.addClass("text-danger");
			...
			$("#nBtnVerifikasi").on("click", function(e) {
				e.preventDefault();
				var data = $('#nfrmvsk').serialize();
				...
				$.ajax({
					type: "POST",
					url: "<?php echo site_url() ?>/arsip/approveBKD",
					data: data,
					success: function(r) {
						$('#vskModal #msg').text(r.pesan)
							.removeClass("text-info")
