...
class M_Kepegawaian extends CI_Model
{
    ...
    public function get_datatables_lihat_dokumen_pns($cariBy,$cariName,$unor)
        ...
        function getProfilPegawaiForKasub($nip){
            ...
            $this->db->select('e.jenis_jabatan,q.nama_status_pegawai,f.id_unitkerjamaster,l.id as id_m_user,l.id_m_sub_bidang,o.nama_bidang,p.nama_sub_bidang,n.nama_kelurahan,m.nama_kecamatan,c.id_tktpendidikan,d.id_pangkat,k.id_statusjabatan,j.id_jenisjab,id_jenispeg,h.id_statuspeg,
            g.id_sk,b.id_agama,e.eselon,j.nm_jenisjab,i.nm_jenispeg,h.nm_statuspeg,g.nm_sk,a.*, b.nm_agama, a.id_m_status_pegawai,
            c.nm_tktpendidikan, d.nm_pangkat, e.nama_jabatan, f.nm_unitkerja, l.id as id_m_user, k.nm_statusjabatan,
            (SELECT CONCAT(aa.nm_jabatan,"|",aa.tmtjabatan,"|",aa.statusjabatan) from db_pegawai.pegjabatan as aa where a.id_peg = aa.id_pegawai and aa.flag_active in (1,2) and aa.status = 2 and aa.statusjabatan not in (2,3) ORDER BY aa.tmtjabatan desc limit 1) as data_jabatan,
        ...
        function getDiklat($nip,$kode){
             $this->db->select('b.id_peg,e.jenjang_diklat,c.created_date,c.keterangan,c.id,c.status,d.nm_jdiklat,c.nm_diklat,
                            c.tptdiklat,c.penyelenggara,c.angkatan,c.jam,c.tglmulai,c.tglselesai,c.tglsttpp,
                            c.nosttpp,c.gambarsk,c.keterangan, c.flag_siasn, c.flag_sync_siasn')
                            ->from('m_user a')
                            ->join('db_pegawai.pegawai b', 'a.username = b.nipbaru_ws')
...
public function getAllPelanggaranByNip($nip){
    return $this->db->select('b.id as id_m_user, a.*, c.nama_pelanggaran_detail')
        ...
        function syncSiasnJabatan($id, $flagCronSync = 0, $dataSync = null){
            $siasn = null;
   ...
   function getVerifLayananPensiun($id){
    ...
    public function loadRiwayatPermohonanCuti(){
        ...
        if($list_id){
            $meta = $this->db->select('a.*')
                        ->from('t_meta_cuti a')
                        ->where_in('a.id_t_pengajuan_cuti', $list_id)
                        ->where('flag_active', 1)
                        ->order_by('tahun', 'desc')
                        ->get()->result_array();
            if($meta){
                foreach($meta as $m){
                    if(isset($result[$m['id_t_pengajuan_cuti']])){
                        $result[$m['id_t_pengajuan_cuti']]['detail'][$m['tahun']] = $m;
                    }
                }
            }
        }
    ...
    public function loadDetailCutiVerif($id){
        ...
        if($result){
            $meta = $this->db->select('a.*')
                        ->from('t_meta_cuti a')
                        ->where('a.id_t_pengajuan_cuti', $result['id'])
                        // ->where('flag_active', 1)
                        ->order_by('tahun', 'desc')
                        ->get()->result_array();
        }
        ...
        if($res['progress']){
            ...
            if($res['progress']['current']['id_m_user_verifikasi'] == $this->general_library->getId()){
                ...
                if($status == 1){ // jika diterima
                    ...
                    if(!$res['progress']['next'] || $res['progress']['next']['id_m_user_verifikasi'] == $this->getDataKabanBkd()['id_m_user']){ // jika kaban yang verif atau selanjutnya kaban, input di usul DS
                        $dataCuti = $this->db->select('a.*, b.nm_cuti, b.nomor_cuti, d.gelar1, d.nama, d.gelar2, d.nipbaru_ws, e.nm_pangkat,
                                f.nama_jabatan, g.nm_unitkerja, a.id as id_t_pengajuan_cuti,
                                g.id_unitkerja, b.id_cuti, c.id as id_m_user, d.id_peg, d.handphone, d.nipbaru_ws')
                                ->from('t_pengajuan_cuti a')
    ...
    public function sendZipSkCuti($zipName, $paramName){
        $exists = $this->db->select('*')
                        ->from('m_parameter')
    ...
    public function deleteNomorSuratManual($id, $tableName = "t_checklist_pensiun"){
        ...
        $mpdf->WriteHTML($html);
        $mpdf->showImageErrors = true;
        $mpdf->Output($usulDs['url'], 'F');

        copy($usulDs['url'], $usulDs['url_ds']);
        ...
        $this->db->where('id', $usulDs['id_t_nomor_surat'])
                ->update('t_nomor_surat', [
                    'flag_active' => 0,
                    'updated_by' => $this->general_library->getId()
                ]);
...
public function getFileForKarisKarsu()
   ...
   public function setPenilaianManajemenTalentaPerPegawai($id_pegawai,$jenis_pengisian,$id){
    ...
    public function getIjazahS1D4Admin($id_peg)
    {
        $this->db->select('*')
        ->where('a.id_pegawai', $id_peg)
        ->where_in('b.id_tktpendidikan', [6000,7000])
    ...
    public function getIjazahAdmin($id_peg)
    {
        $this->db->select('*')
        ->where('id_pegawai', $id_peg)
        ->where('flag_active', 1)
    ...
    public function getIjazahTerakhirAdmin($id_peg)
    {
     ...
     public function getIjazahTerakhir2Admin($id_peg)
    {
        $this->db->select('*')
        ->where('id_pegawai', $id_peg)
        ->where('flag_active', 1)
    ...
    public function getIjazahPAdmin($id_peg)
    {
        $this->db->select('*')
        ->where('id_pegawai', $id_peg)
        ->where('flag_active', 1)
    ...
    public function getDokumenJabatanTerakhirForLayananAdmin($id_peg)
    {
        $this->db->select('*')
        ->from('db_pegawai.pegjabatan as a')
        ->join('db_pegawai.jabatan b', 'b.id_jabatanpeg = a.id_jabatan')
    ...
    public function getDokumenAkteNikahForPensiun()
    {
        $this->db->select('*')
        ->where('id_pegawai', $this->general_library->getIdPegSimpeg())
        ->where('flag_active', 1)
        ->where('status !=', 3)
        ->where_in('hubkel', [20,30])
        ->order_by('created_date', 'desc')
        ->limit(1)
...
public function getFileForVerifLayanan()
    {      
        ...
        if($this->input->post('file') == "skcpns"){
           $sk = $this->db->select('a.gambarsk')
            ...
            $this->db->select('a.gambarsk')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "rekom_kepala_pd"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 79)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        ...
        $this->db->select('a.gambarsk')
            ...
            $this->db->select('a.gambarsk')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "ijazah"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegpendidikan as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "ijazah2"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegpendidikan as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
            ...
            $this->db->select('a.gambarsk')
                ...
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.ijazah_s_penyesuaian', 1)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "ijazah_penyesuaian"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegpendidikan as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.ijazah_penyesuaian', 1)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
            ...
            $this->db->select('a.gambarsk')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "transkrip"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 77)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "akreditasi_prodi"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
            ...
            $this->db->select('a.surat_pernyataan_tidak_cltn')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "surat_ket_lulus_mhs"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 85)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
            ...
            $this->db->select('a.gambarsk')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "krs"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 89)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "rencana_pengembangan_diri"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 90)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "suket_beasiswa"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 87)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "suket_online"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 88)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "sk_penetapan_prestasi"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 93)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "bukti_prestasi"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 94)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "surat_pelantikan_jabatan"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 98)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "spmt"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 34)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "cv"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 95)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
            ...
            $this->db->select('a.gambarsk')
                ...
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 96)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "berita_acara_jabatan"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 97)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
            ...
            $this->db->select('a.gambarsk')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "ijazahd4s1"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegpendidikan as a')
                ->join('db_pegawai.tktpendidikanb as b ', 'a.tktpendidikan = b.id_tktpendidikanb')
                ->where('a.id_pegawai', $id_peg)
            ...
            $this->db->select('a.gambarsk')
                ...
                ->limit(1);
                return $this->db->get()->result_array();
        } else if($this->input->post('file') == "sk_jf_asal"){
            $this->db->select('a.gambarsk')
                ->from('db_pegawai.pegarsip as a')
                ->where('a.id_pegawai', $id_peg)
                ->where('a.flag_active', 1)
                ->where('a.id_dokumen', 104)
                ->where('a.status !=', 3)
                ->order_by('a.created_date', 'desc')
                ->limit(1);
                return $this->db->get()->result_array();
    ...
    public function submitVerifikasiPengajuanLayanan(){
        ...
        if($dataPengajuan[0]['id_m_layanan'] == 6 || $dataPengajuan[0]['id_m_layanan'] == 7 || $dataPengajuan[0]['id_m_layanan'] == 8 || $dataPengajuan[0]['id_m_layanan'] == 9){
            ...
            $jenislayanan = " Surat Keterangan / Pernyataan";
        } else if($dataPengajuan[0]['id_m_layanan'] == 27){
            $message = "*[ADMINISTRASI KEPEGAWAIAN - LAYANAN SURAT REKOMENDASI MENGIKUTI SELEKSI MASUK PERGURUAN TINGGI]*\n\nSelamat ".greeting()." ".getNamaPegawaiFull($dataPengajuan[0]).".\nPengajuan Layanan Surat Rekomendasi Mengikuti Seleksi Masuk Perguruan Tinggi anda tanggal ".formatDateNamaBulan($dataPengajuan[0]['tanggal_usul'])." telah ".$statusForMessage.".\n\nStatus: ".$status."\nCatatan Verifikator : ".$dataPengajuan[0]['keterangan']."\n\nTerima Kasih\n*BKPSDM Kota Manado*";
            $jenislayanan = " Surat Keterangan / Pernyataan";
    ...
    return $res;
     ...
     public function insertUsulLayananTubel($id_m_layanan)
	{
        ...
        if($cek){
            ...
            if(isset($_FILES['file2']['name'])){
                $filehd =  "surat_pernyataan_tidak_hd_$nip"."_$random_number".".pdf";
                $target_dir_hd	= './dokumen_layanan/tugasbelajar/surat_ket_hd';
            }

            if(isset($_FILES['file3']['name'])){
               $filecltn =  "surat_pernyataan_tidak_cltn_$nip"."_$random_number".".pdf";
               $target_dir_cltn	= './dokumen_layanan/tugasbelajar/suket_cltn';
            } 
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                return $res;
                    ...
                    $dataUsul['file_pengantar']      = "$nama_file.pdf";
                    $dataUsul['surat_pernyataan_tidak_hd']      = $filehd;
                    $dataUsul['surat_pernyataan_tidak_cltn']      = $filecltn;
            ...
            if(isset($_FILES['file3']['name'])){
                $config_pidana['upload_path']       = $target_dir;
                $config_pidana['allowed_types']     = 'pdf';
                $config_pidana['encrypt_name']		= FALSE;
                $config_pidana['overwrite']			= TRUE;
                $config_pidana['detect_mime']		= TRUE;
                $config_pidana['file_name']         = $filecltn;
                $this->upload->initialize($config_pidana);
                if (!$this->upload->do_upload('file3')) {
                    $data['error']    = strip_tags($this->upload->display_errors());
                    $data['token']    = $this->security->get_csrf_hash();
    ...
    public function cekProsesKenaikanBerkala($id_pegawai,$tahun){
        $this->db->select('a.*,b.url_ds')
                ->from('t_gajiberkala a')
                ->join('t_usul_ds b', 'a.id = b.ref_id', 'left')
                ->where('a.tahun', $tahun)
    ...
    public function simpanDataDrafKgb($no_sk){
...
public function checkListIjazahCpns($id, $id_pegawai){
    ...
    public function laporanJumlahPegawaiMenurutGolongan(){
        ...
        foreach($temp_pangkat as $pang){
            ...
            $result['pangkat'][$pang['id_pangkat']]['nama'] = $pang['nm_pangkat'];
    ...
    public function laporanJumlahPegawaiMenurutJabatan(){
        ...
        $this->db->select('a.jk,c.eselon,d.id_unitkerjamaster,c.jenis_jabatan, a.nipbaru_ws')
            ->from('db_pegawai.pegawai a')
                    ->join('db_pegawai.unitkerja b', 'a.skpd = b.id_unitkerja')
                    ->join('db_pegawai.jabatan c', 'a.jabatan = c.id_jabatanpeg', 'left')
    ...
    public function laporanJumlahPegawaiFungsionalMenurutGolKategori(){
            ...
            $result['jabatan'][6]['nama'] = 'III/d';
            $result['jabatan'][6]['utama'] = 0;
            $result['jabatan'][6]['madya'] = 0;
            $result['jabatan'][6]['muda'] = 0;
            $result['jabatan'][6]['pertama'] = 0;
            $result['jabatan'][6]['penyelia'] = 0;
            $result['jabatan'][6]['mahir'] = 0;
            $result['jabatan'][6]['terampil'] = 0;
            $result['jabatan'][6]['pemula'] = 0;

            $result['jabatan'][7]['nama'] = 'III/c';
            $result['jabatan'][7]['utama'] = 0;
            $result['jabatan'][7]['madya'] = 0;
            $result['jabatan'][7]['muda'] = 0;
            $result['jabatan'][7]['pertama'] = 0;
            $result['jabatan'][7]['penyelia'] = 0;
            $result['jabatan'][7]['mahir'] = 0;
            $result['jabatan'][7]['terampil'] = 0;
            $result['jabatan'][7]['pemula'] = 0;

            $result['jabatan'][8]['nama'] = 'III/b';
            $result['jabatan'][8]['utama'] = 0;
            $result['jabatan'][8]['madya'] = 0;
            $result['jabatan'][8]['muda'] = 0;
            $result['jabatan'][8]['pertama'] = 0;
            $result['jabatan'][8]['penyelia'] = 0;
            $result['jabatan'][8]['mahir'] = 0;
            $result['jabatan'][8]['terampil'] = 0;
            $result['jabatan'][8]['pemula'] = 0;

            $result['jabatan'][9]['nama'] = 'III/a';
            $result['jabatan'][9]['utama'] = 0;
            $result['jabatan'][9]['madya'] = 0;
            $result['jabatan'][9]['muda'] = 0;
            $result['jabatan'][9]['pertama'] = 0;
            $result['jabatan'][9]['penyelia'] = 0;
            $result['jabatan'][9]['mahir'] = 0;
            $result['jabatan'][9]['terampil'] = 0;
            $result['jabatan'][9]['pemula'] = 0;

            $result['jabatan'][10]['nama'] = 'II/d';
            $result['jabatan'][10]['utama'] = 0;
            $result['jabatan'][10]['madya'] = 0;
            $result['jabatan'][10]['muda'] = 0;
            $result['jabatan'][10]['pertama'] = 0;
            $result['jabatan'][10]['penyelia'] = 0;
            $result['jabatan'][10]['mahir'] = 0;
            $result['jabatan'][10]['terampil'] = 0;
            $result['jabatan'][10]['pemula'] = 0;

            $result['jabatan'][11]['nama'] = 'II/c';
            $result['jabatan'][11]['utama'] = 0;
            $result['jabatan'][11]['madya'] = 0;
            $result['jabatan'][11]['muda'] = 0;
            $result['jabatan'][11]['pertama'] = 0;
            $result['jabatan'][11]['penyelia'] = 0;
            $result['jabatan'][11]['mahir'] = 0;
            $result['jabatan'][11]['terampil'] = 0;
            $result['jabatan'][11]['pemula'] = 0;

            $result['jabatan'][12]['nama'] = 'II/b';
            $result['jabatan'][12]['utama'] = 0;
            $result['jabatan'][12]['madya'] = 0;
            $result['jabatan'][12]['muda'] = 0;
            $result['jabatan'][12]['pertama'] = 0;
            $result['jabatan'][12]['penyelia'] = 0;
            $result['jabatan'][12]['mahir'] = 0;
            $result['jabatan'][12]['terampil'] = 0;
            $result['jabatan'][12]['pemula'] = 0;

            $result['jabatan'][13]['nama'] = 'II/a';
            $result['jabatan'][13]['utama'] = 0;
            $result['jabatan'][13]['madya'] = 0;
            $result['jabatan'][13]['muda'] = 0;
            $result['jabatan'][13]['pertama'] = 0;
            $result['jabatan'][13]['penyelia'] = 0;
            $result['jabatan'][13]['mahir'] = 0;
            $result['jabatan'][13]['terampil'] = 0;
            $result['jabatan'][13]['pemula'] = 0;

            $result['jabatan'][14]['nama'] = 'I/d';
            $result['jabatan'][14]['utama'] = 0;
            $result['jabatan'][14]['madya'] = 0;
            $result['jabatan'][14]['muda'] = 0;
            $result['jabatan'][14]['pertama'] = 0;
            $result['jabatan'][14]['penyelia'] = 0;
            $result['jabatan'][14]['mahir'] = 0;
            $result['jabatan'][14]['terampil'] = 0;
            $result['jabatan'][14]['pemula'] = 0;

            $result['jabatan'][15]['nama'] = 'I/c';
            $result['jabatan'][15]['utama'] = 0;
            $result['jabatan'][15]['madya'] = 0;
            $result['jabatan'][15]['muda'] = 0;
            $result['jabatan'][15]['pertama'] = 0;
            $result['jabatan'][15]['penyelia'] = 0;
            $result['jabatan'][15]['mahir'] = 0;
            $result['jabatan'][15]['terampil'] = 0;
            $result['jabatan'][15]['pemula'] = 0;

            $result['jabatan'][16]['nama'] = 'I/b';
            $result['jabatan'][16]['utama'] = 0;
            $result['jabatan'][16]['madya'] = 0;
            $result['jabatan'][16]['muda'] = 0;
            $result['jabatan'][16]['pertama'] = 0;
            $result['jabatan'][16]['penyelia'] = 0;
            ...
            $result['jabatan'][17]['madya'] = 0;
            $result['jabatan'][17]['muda'] = 0;
            $result['jabatan'][17]['pertama'] = 0;
            $result['jabatan'][17]['penyelia'] = 0;
        ...
        if($peg['pangkat'] == "45" ) {
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][1]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][1]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        ...
        $result['jabatan'][2]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][2]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][2]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        $result['jabatan'][3]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][3]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][3]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][3]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][3]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        $result['jabatan'][4]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][4]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][4]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][4]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][4]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        $result['jabatan'][5]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][5]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][5]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        $result['jabatan'][6]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][6]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][6]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][6]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][6]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        $result['jabatan'][7]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][7]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][7]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][7]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][7]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][8]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][8]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][8]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][8]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][8]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][8]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][9]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][9]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][9]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][9]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][9]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][9]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][10]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][10]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][10]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][10]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][10]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][10]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][11]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][11]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][11]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][11]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][11]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][11]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][12]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][12]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][12]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][12]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][12]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][12]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][13]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][13]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][13]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][13]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][13]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][13]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][14]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][14]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][14]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][14]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][14]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][14]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][15]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][15]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][15]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][15]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][15]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][15]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][16]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][16]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][16]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][16]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][16]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][16]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
        ...
        if(substr($peg['nama_jabatan'], -5) == "Utama"){
        $result['jabatan'][17]['utama']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Madya") {
        $result['jabatan'][17]['madya']++;
        } else if(substr($peg['nama_jabatan'], -4) == "Muda") {
        $result['jabatan'][17]['muda']++;
        } else if(substr($peg['nama_jabatan'], -7) == "Pertama") {
        $result['jabatan'][17]['pertama']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Penyelia") {
        $result['jabatan'][17]['penyelia']++;
        } else if(substr($peg['nama_jabatan'], -5) == "Mahir") {
        $result['jabatan'][17]['mahir']++;
        } else if(substr($peg['nama_jabatan'], -8) == "Terampil") {
    ...
    public function insertUsulLayananMutasiAsn($id_m_layanan)
	{
    //    dd($_FILES);
        ...
        $cek =  $this->db->select('*')
        ->from('t_layanan a')
        ->where('a.id_m_user', $this->input->post('pegawai'))
        ->where('a.flag_active', 1)
        ->where('a.id_m_layanan', $id_m_layanan)
        ->where_in('a.status', [0,2])
        ->get()->result_array();
        ...
        if($cek){
            ...
            $nip = $this->input->post('nip');
            $random_number = intval( "0" . rand(1,9) . rand(0,9) . rand(0,9) . rand(0,9) . rand(0,9) );
                $nama_file = "rekom_$nip"."_$random_number";
                $target_dir	= './dokumen_layanan/mutasiasn';
            ...
            $this->load->library('upload');
            if(isset($_FILES['file2']['name'])){
                $filepj =  "peta_jab_$nip"."_$random_number".".pdf";
                $target_dir_hd	= './dokumen_layanan/mutasiasn';
            }
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                return $res;
                    ...
                    $dataFile 			= $this->upload->data();
                    $dataUsul['id_m_user']      = $this->input->post('pegawai');
                    $dataUsul['created_by']      = $this->general_library->getId();
                    $dataUsul['id_m_layanan']      = $id_m_layanan;
                    $dataUsul['file_pengantar']      = "$nama_file.pdf";
                    $dataUsul['peta_jabatan']      = $filepj;
                    $this->db->insert('db_efort.t_layanan', $dataUsul);
                    $res = array('msg' => 'Data berhasil disimpan', 'success' => true);
            }
    ...
    function getRiwayatLayananMutasiAsn($id){
        $this->db->select('*, a.created_date as tanggal_usul, a.id as id_t_layanan')
                       ->from('t_layanan a')
                       ->join('m_user b', 'a.id_m_user = b.id')
                    //    ->where('a.id_m_user', $this->general_library->getId())
