...
class M_Kepegawaian extends CI_Model
{
    ...
    public function get_datatables_lihat_dokumen_pns($cariBy,$cariName,$unor)
        ...
        function getJabatan($nip,$kode){
              $this->db->select('c.statusjabatan,c.id_pegawai,c.created_date,c.id,c.status,c.nm_jabatan,d.nama_jabatan,c.tmtjabatan,c.angkakredit, e.nm_eselon,c.skpd,c.nosk,c.tglsk,c.ket,c.gambarsk,c.keterangan')
                            ->from('m_user a')
                            ->join('db_pegawai.pegawai b','a.username = b.nipbaru_ws')
...
public function submitEditJabatan(){
    ...
    public function getProgressCutiAktif($id){
        ...
        if($progress){
            ...
            if(isset($progress[$result['current']['urutan']])){
    ...
    public function verifPermohonanCutiFromWa($resp, $chat){
        ...
        } else if($progress['current']['nohp'] != $nomor_sender){
    ...
    public function checkIfReplyCuti($chat){
        ...
        $result['wa'] = $this->db->select('*')
                        ...
                        ->get()->row_array();
        if($result['wa']){
            $result['cuti'] = $this->db->select('a.*, c.handphone, c.gelar1, c.gelar2, c.nama, d.nm_cuti')
